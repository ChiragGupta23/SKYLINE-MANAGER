 ...
 * ### Type plugin
 *
 * Makes it possible to add predefined Type for groups of nodes, which make it possible to easily control nesting rules and icon for each group.
 */

	/**
	 * An object storing all Type as key value pairs, where the key is the type name and the value is an object that could contain following keys (all optional).
	 *
	 ...
	 *
	 * There are two predefined Type:
	 *
	 * * `#` represents the root of the tree, for example `max_children` would control the maximum number of root nodes.
	 * * `default` represents the default node - any settings here will be applied to all nodes that do not have a type specified.
	 *
	 * @name $.jstree.defaults.Type
	 * @plugin Type
	 */
	$.jstree.defaults.Type = {
		'default' : {}
	};
	$.jstree.defaults.Type[$.jstree.root] = {};

	$.jstree.plugins.Type = function (options, parent) {
		this.init = function (el, options) {
			var i, j;
			if(options && options.Type && options.Type['default']) {
				for(i in options.Type) {
					if(i !== "default" && i !== $.jstree.root && options.Type.hasOwnProperty(i)) {
						for(j in options.Type['default']) {
							if(options.Type['default'].hasOwnProperty(j) && options.Type[i][j] === undefined) {
								options.Type[i][j] = options.Type['default'][j];
							}
						}
					}
				}
				...
				case "copy_node":
					if(chk !== 'move_node' || $.inArray(obj.id, par.children) === -1) {
						...
						if(tmp.max_children !== undefined && tmp.max_children !== -1 && tmp.max_children === par.children.length) {
							this._data.core.last_error = { 'error' : 'check', 'plugin' : 'Type', 'id' : 'Type_01', 'reason' : 'max_children prevents function: ' + chk, 'data' : JSON.stringify({ 'chk' : chk, 'pos' : pos, 'obj' : obj && obj.id ? obj.id : false, 'par' : par && par.id ? par.id : false }) };
							return false;
						}
						if(tmp.valid_children !== undefined && tmp.valid_children !== -1 && $.inArray((obj.type || 'default'), tmp.valid_children) === -1) {
							this._data.core.last_error = { 'error' : 'check', 'plugin' : 'Type', 'id' : 'Type_02', 'reason' : 'valid_children prevents function: ' + chk, 'data' : JSON.stringify({ 'chk' : chk, 'pos' : pos, 'obj' : obj && obj.id ? obj.id : false, 'par' : par && par.id ? par.id : false }) };
							return false;
						}
						...
						do {
							if(tmp.max_depth !== undefined && tmp.max_depth !== -1 && tmp.max_depth < d) {
								this._data.core.last_error = { 'error' : 'check', 'plugin' : 'Type', 'id' : 'Type_03', 'reason' : 'max_depth prevents function: ' + chk, 'data' : JSON.stringify({ 'chk' : chk, 'pos' : pos, 'obj' : obj && obj.id ? obj.id : false, 'par' : par && par.id ? par.id : false }) };
								return false;
							}
							par = this.get_node(par.parent);
							tmp = this.get_rules(par);
		...
		this.get_type = function (obj, rules) {
			obj = this.get_node(obj);
			return (!obj) ? false : ( rules ? $.extend({ 'type' : obj.type }, this.settings.Type[obj.type]) : obj.type);
		};
		/**
	...
	// include the Type plugin by default
	// $.jstree.defaults.plugins.push("Type");
